...
class SearchPoem extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return SearchPoemState();
  }
}

class SearchPoemState extends State<SearchPoem> {
  List<Poem> list = [];
  ...
  Future<void> editPoem(Poem poem) async {
    ...
    await db.queryPoemsByString(searchStr).then((value) {
      setState(() {
        List<Poem> _list = [];
        list = _list;
                    ...
                    Container(
                        ...
                        child: TextField(
                          ...
                          onChanged: (str) async {
                            if (str.length != 0) {
                              List<Poem> newList = [];
                              await db.queryPoemsByString(str).then((value) {
                                value.forEach((element) {
                                  newList.add(Poem(
                                      element['id'] as int,
                                      element['title'] as String,
