...
import 'package:my_feeling/readPoem.dart';

class DownloadPoem extends StatefulWidget {
  var list;

  DownloadPoem(this.list);

  @override
  State<StatefulWidget> createState() {
    return DownloadPoemState(list);
  }
}

class DownloadPoemState extends State<DownloadPoem> {
  late List<Poem> list;
  DBManager db = DBManager();
  List<bool> downloadList = [];

  DownloadPoemState(this.list);
                    ...
                    await showDeleteConfirmDialog1().then((value) async {
                      if (value == true) {
                        for (int i = 0; i < downloadList.length; i++) {
                          if (downloadList[i]) {
                            Poem tmp = list[i];
                            await db.addPoem(tmp.title, tmp.group, tmp.content,
                                tmp.datetime);
                          }
                        }
                      }
                      ...
                      child: GestureDetector(
                        ...
                        onTap: () {
                          Navigator.push(context,
                            ...
                            return ReadPoem(poem: list[index]);
                          }));
                        },
                        child: Container(
